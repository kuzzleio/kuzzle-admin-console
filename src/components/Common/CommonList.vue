...
export default {
  ...
  watch: {
    $route: {
      ...
      handler(newValue) {
        ...
        filterManager.addNewHistoryItemAndSave(newValue, this.index, this.collection);
      },
    },
    currentFilter() {
      this.fetchDocuments();
  ...
  methods: {
    ...
    fetchDocuments() {
      ...
      this.performSearch(this.collection, this.index, searchQuery, pagination, sorting)
        .then((res) => {
