...
const isActionAllowed = (user, controller, action, index = '*', collection = '*') => {
  if (!user) {
    return false;
  }
...
export const getters = defineGetters<AuthState>()({
  ...
  canManageRealtime(state, getters) {
    return (index, collection) =>
      getters.canSubscribe(index, collection) || getters.canPublish(index, collection);
  },
  ...
  hasSecurityRights(state, getters) {
    return getters.getterscanManageRoles || getters.canManageProfiles || getters.canManageUsers;
  },
